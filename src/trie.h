...
#ifdef _WIN32
#    define TRIE_DLLEXPORT __declspec(dllexport)
#elif __GNUC__ >= 4
#  define TRIE_DLLEXPORT __attribute__ ((visibility("default")))
#else
#  define TRIE_DLLEXPORT
#endif
...
TRIE_DLLEXPORT trie_t *trie_new(void);
...
TRIE_DLLEXPORT bool trie_get_data(trie_t *self, char *key, uint32_t *data);
...
TRIE_DLLEXPORT bool trie_add(trie_t *self, char *key, uint32_t data);
...
TRIE_DLLEXPORT void trie_destroy(trie_t *self);
