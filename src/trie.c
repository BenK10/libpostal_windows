...
TRIE_DLLEXPORT inline bool trie_add(trie_t *self, char *key, uint32_t data) {
...
TRIE_DLLEXPORT inline bool trie_get_data(trie_t *self, char *key, uint32_t *data) {
...
trie_t *trie_load(char *path) {
    ...
    file = fopen(path, "rb"); 
    if (!file)
	    return NULL;  //DEBUG added perror
